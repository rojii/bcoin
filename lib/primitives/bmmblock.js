/*!
 * bmmblock.js - bmm block object for testchain
 ...
 * @alias module:primitives.BMMBlock
 * @abstract
 * @property {Number} version
 * @property {Hash} prevBlock
 * @property {Hash} merkleRoot
 * @property {Hash} withdrawalHash
 * @property {Hash} mainchainHash
...
class BMMBlock {
  ...
  constructor() {
    ...
    // bmm related only
    this.withdrawalHash = consensus.ZERO_HASH;
    this.mainchainHash = consensus.ZERO_HASH;
  ...
  parseOptions(options) {
    ...
    assert(Buffer.isBuffer(options.withdrawalHash));
    assert(Buffer.isBuffer(options.mainchainHash));
    ...
    this.withdrawalHash = options.withdrawalHash;
    this.mainchainHash = options.mainchainHash;
  ...
  parseJSON(json) {
    ...
    assert(typeof json.withdrawalHash === 'string');
    assert(typeof json.mainchainHash === 'string');
    ...
    this.withdrawalHash = util.fromRev(json.withdrawalHash);
    this.mainchainHash = util.fromRev(json.mainchainHash);
    this.time = json.time;
    this.bits = json.bits;
  ...
  /**
   * Serialize the block headers. Size increased with extra 32byte
   * entries added to the block headers
   * @returns {Buffer}
   */
  ...
  writeHead(bw) {
    ...
    bw.writeHash(this.withdrawalHash);
    bw.writeHash(this.mainchainHash);
  ...
  readHead(br) {
    ...
    this.withdrawalHash = br.readHash();
    this.mainchainHash = br.readHash();
